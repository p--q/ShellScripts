...
echo "--- KVM共存設定 (Version 1.0.0) を開始します ---"

# 2. 設定ファイルの作成
# enable_virt_at_load=0 により、KVMが必要になるまでCPUの仮想化機能をロックしないようにします
...
echo "[3/3] カーネル設定(initramfs)を更新中...（1〜2分かかる場合があります）"
if update-initramfs -u; then
    echo "--- 設定完了！ ---"
    echo "変更を反映させるために、システムを再起動してください。"
else
    echo "エラー: initramfsの更新に失敗しました。パスを確認してください。"
    exit 1
fi
