...
# Description: 画質劣化ゼロ(WebP Lossless)でSXGAサイズ(1280x1024)へ変換。
#              ファイル名末尾に _sxga を付与。透明背景は白で塗りつぶし。
# Version:     1.4.0
# ==============================================================================
...
(
    ...
    for FILE in "$@"; do
        ...
        # 出力ファイル名の決定（末尾に _sxga を付与）
        OUT_FILE="$DIR/${BASENAME}_sxga.webp"
        
        # 同名回避ロジック（_sxga_01, _sxga_02...）
        if [ -f "$OUT_FILE" ]; then
            I=1
...
) | zenity --progress --title="SXGAロスレス変換" --text="準備中..." --auto-close --percentage=0
